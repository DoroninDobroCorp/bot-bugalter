...
class Employee(Model):
	...
	async def penalty(self, reports_employee=None):
		...
		for report_employee in reports_employee:
			...
			if not report:
				continue
			# Бизнес-правило: штраф не делится, целиком на ответственного сотрудника.
			# Ответственным считаем первого в списке report.employees.
			if report.is_error and report.profit < 0:
				if report.employees and report.employees[0].id == self.id:
					penalty_sum += report.penalty
				# Если сотрудник не первый (или нет сотрудников) — штраф не начисляем.
			else:
				# Для неошибочных отчётов штрафа нет по текущей логике Report.penalty
				# (оставляем на случай расширений логики в будущем)
				penalty_sum += 0
	...
	async def get_balance(self):
		async with async_session() as session:
			...
			for report in reports:
				emp_count = len(report.employees) if report.employees else 1
				# Зарплата: если отчёт не ошибочный — делим на число сотрудников
				if not report.is_error:
					salary_sum += (report.salary / emp_count)
				# Штраф: только если отчёт ошибочный и убыток; начисляем полностью первому сотруднику
				if report.is_error and report.profit < 0:
					if report.employees and report.employees[0].id == self.id:
						penalty_sum += report.penalty
...
class Report(Model):
	...
	@property
	def salary(self):
		bet = self.bet_amount or 0
		return 0 if self.is_error else (bet * self.salary_percentage / 100)
